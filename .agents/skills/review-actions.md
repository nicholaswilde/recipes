...
Monitor GitHub Actions workflow runs, diagnose failure logs, and resolve issues (linting, spellcheck,
link check, or build failures).

## Description

This skill outlines how to list workflow runs, inspect detailed job logs, and apply automated or manual fixes
for typical repository failures. All `gh` commands must be piped to `cat` to bypass interactive pagination.
